...
5. consult @.opencode/workflows/baicai-vc/phase-log-memory/read-context.md only when continuity context is useful
6. derive the step-scoped test checklist and chosen test-layer mode from the extracted step context using @.opencode/rules/baicai-vc/test-layer-mode.md
...
12. call @.opencode/workflows/baicai-vc/write-step-e2e.md to write or refresh canonical E2E only when the chosen test-layer mode is `lower-level + e2e`
...
16. follow @.opencode/rules/baicai-vc/review-flow.md for the requirements-review retry loop and exit behavior
...
18. invoke `@simplify-code` from @.opencode/agents/baicai-vibe/simplify-code.md on the touched code only, preserving behavior and step scope
19. follow @.opencode/rules/baicai-vc/review-flow.md for quality-review batch selection, delta verification, stop reasons, and exit behavior
...
20. call @.opencode/workflows/baicai-vc/update-step-tracking.md only when requirements review is `correct` and the latest quality round is clean with no unresolved required findings
