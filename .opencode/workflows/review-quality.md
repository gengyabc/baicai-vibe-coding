...
3. consult @.opencode/workflows/phase-log-memory/read-context.md only when the review artifact points to missing continuity context
4. if @.planning/phase/<step-folder>/test-checklist.md is missing, derive the step-scoped test checklist and chosen test-layer mode from the step context using @.opencode/rules/test-layer-mode.md, then save or refresh the artifact before verification
...
13. invoke `@quality-review` in `delta` mode, then follow @.opencode/rules/review-flow.md for verification, stop reasons, and next batch selection
...
14. call @.opencode/workflows/update-step-tracking.md only when the caller has a clean terminal review state
